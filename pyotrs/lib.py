...
class Ticket(object):
    ...
    def create_basic(cls,
        ...
        if Service and ServiceID:
            raise ArgumentInvalidError("Either Service or ServiceID - not both")

        if SLA and SLAID:
            raise ArgumentInvalidError("Either SLA or SLAID - not both")
        ...
        if Service:
            dct.update({"Service": Service})
        if ServiceID:
            dct.update({"ServiceID": ServiceID})

        if SLA:
            dct.update({"SLA": SLA})
        if SLAID:
            dct.update({"SLAID": SLAID})
